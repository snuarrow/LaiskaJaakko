...
class SensorMonitor:  # Has MoistureSensor and SensorHistory, periodicaly reads data from MoistureSensor and stores it in SensorHistory
    def __init__(self, sensor, history) -> None:
        ...
        self._record_data()
        self.timer.init(period=60000, mode=Timer.PERIODIC, callback=self._record_data)
...
def handle_request(conn):
    ...
    #    return
        ...
        if "?sensor_index=" in request.split(" ")[1]:
            ...
            print(len(sensors.sensor_monitors))
            listed_sensors = []

            for index in range(len(sensors.sensor_monitors)):
                print(f"get sensor index: {index}")
                values = []
                times = []
                for elem in sensors.get_sensor(index=index).get_data():
                    values.append(elem[0])
                    times.append(elem[1])
                listed_sensors.append(
                    {
                        "type": sensors.get_sensor(index=index).history.sensor_type,
                        "values": values,
                        "times": times,
                    }
                )
...
storage_sensor_history = SensorHistory(
    "storage.log", 60, pico_timer, sensor_type="storage"
)
...
memory_sensor_history = SensorHistory(
    "memory.log", 60, pico_timer, sensor_type="memory"
)
